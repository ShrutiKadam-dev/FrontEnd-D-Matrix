...
export class FixedIncomeComponent {
  ...
  goToFixedIncomeDetails(e: any) {
    ...
    switch (e.subcategory?.toLowerCase()) {
      case 'etf':
        this.router.navigate(['/features/fixed-income/ETF']);
        break;

      case 'direct equity':
        this.router.navigate(['/features/fixed-income/direct-equity']);
        break;

      case 'alternative investment funds':
        this.router.navigate(['/features/fixed-income/AIF']);
        break;
  ...
  getAllFixedIncomeHome() {
    this.featuresService.getAllFixedIncomeHome().subscribe({
      next: (res: any) => {
        this.allEs = res?.data || [];
        // Assign colors once
